...
use phpchassis\lib\http\middleware\file\UploadedFile;
use phpchassis\lib\http\middleware\Constants;
...
class ServerRequest extends Request implements ServerRequestInterface {
   ...
   * which is not in PSR-7, but is extremely convenient.
   *
   * @return $this
   */
...
// --------------------------------------------------------------------------------------
// As with the other classes defined previously, we provide with methods that add or
// overwrite properties and return the new instance:
// --------------------------------------------------------------------------------------
  ...
  /**
   * One important aspect of PSR-7 messages is that the body should also be available in a parsed manner,
   * that is to say, a sort of structured representation rather than just a raw stream. Accordingly, we define
   * getParsedBody() and its accompanying with method. The PSR-7 recommendations are quite specific when it comes
   * to form posting.
   *
   * @return mixed
   */
...
// ------------------------------
  ...
  public function __get(string $propertyName) {
    return ($this->hasFile($propertyName)) ? $this->uploadedFileObjs[$propertyName] : $this->parsedBody[$propertyName];
  }
