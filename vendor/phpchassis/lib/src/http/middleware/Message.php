...
class Message implements MessageInterface {
  ...
  /**
   * Returns the current Message instance and allows us to overwrite the current value of body
   *
  ...
  /**
   * PSR-7 recommends that headers should be viewed as case-insensitive. Accordingly,
  ...
  /**
   * Although not defined by PSR-7, this method is designed to populate the $httpHeaders
   * property. This property is assumed to be an associative array where the key is the
   ...
   *      There is an excellent apache_request_headers() PHP function from the Apache
   *      extension that produces headers if they are not already available in $httpHeaders:
   *
   * @return array
   */
  ...
  /**
   * If apache_request_headers() is not available (that is, the Apache extension is not enabled),
   * we provide an alternative, altApacheReqHeaders().
   *
   * @return array
   */
  ...
  /**
   * Implementing getHeaders() (required in PSR-7) is now a trivial loop through the $httpHeaders property
   * produced by the getHttpHeaders() method
   */
...
// -----------------------------------------------------------------------------------
// Again, we provide a series of with methods designed to overwrite or replace
...
// -----------------------------------------------------------------------------------
  ...
  public function withHeader($name, $value) {
    ...
    // findHeader() allow for case-insensitive handling of headers
...
// --------------------------------------------------------------------------------------
// A series of useful header-related methods to confirm a header exists,
// retrieve a single header line, and retrieve a header in array form, as per PSR-7:
// --------------------------------------------------------------------------------------
   ...
   * only be the numerical part. For this reason, we also provide onlyVersion() that strips off any non-digit
