...
class Uri implements UriInterface {
  ...
  /**
   * User info represents the username (if present) and optionally the password. An example of when a password is
   * used is when accessing an FTP website such as:
  ...
  /**
   * Port is the HTTP port, if present. You will note if a port is listed in our STANDARD_PORTS constant, the return
   * value is NULL, according to the requirements of PSR-7.
   *
   * @return int|null
   */
   ...
   * rawurlencode() PHP function as it is compliant with RFC 3986. We cannot just encode the entire path, however,
   * as the path separator (that is, /) would also get encoded! Accordingly, we need to first break it up using  544
   * explode(), encode the parts, and then reassemble it.
   *
   * @return string
   */
  ...
  /**
   * This method retrieves the query string (that is, from $_GET). These too must be URL-encoded. First, we define
   * getQueryParams(), which breaks the query string into an associative array. You will note the reset option in case
   * we wish to refresh the query parameters. We then define getQuery(), which takes the array and produces a proper
   * URL-encoded string.
   *
   * @param bool $reset
   * @return array
  ...
  /**
   * When the object is used in a string context, this method returns a proper URI, assembled from $uriParts. We also
   * define a convenience method, getUriString(), that simply calls __toString().
   *
   * @return string
   */
  public function __toString() {
    ...
    // Before adding path, we first check whether the first character is /. If not, we need to
    // add this separator...
    $path = $this->getPath();
  ...
  /* -------------------------------------------------------------------------------------
  Here, we define a series of withXXX() methods, which match the getXXX()
  methods described above. These methods are designed to add, replace, or remove
  properties associated with the request class (scheme, authority, user info, and so on).
  In addition, these methods return the current instance that allows us to use these
  methods in a series of successive calls (often referred to as the fluent interface).
  ----------------------------------------------------------------------------------------*/
